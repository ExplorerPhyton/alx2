...
let textLabel: HTMLDivElement | null = null;
...
sphereData.forEach((data, index) => {
  ...
  (sphere as any).labelText = data.text; // Attach text to sphere
  (sphere as any).sphereIndex = index; // Store index
  scene.add(sphere);
  spheres.push(sphere);
});
...
window.addEventListener('click', (event) => {
  ...
  if (intersects.length > 0) {
    ...
    isAnimating = true;
    animationProgress = 0;
    createTextLabel((selectedSphere as any).labelText);
  } else {
    if (selectedSphere) {
      // Reset zoom
      isAnimating = true;
